...
/*   Updated: 2024/07/25 20:22:51 by almichel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		char *file, char *redir, t_data *data)
{
	...
	if (pid == 0)
	{
		...
		check_redirection(redir, file, &fd);
		if (fd > 0)
			check_and_exe_cmd(cur, env, exp_var, fd, redir, data);
		exit(127);
	}
	else if (pid > 0)
	{
...
void	check_and_exe_cmd(t_token *cur, t_list **envp, t_list **exp_var, int fd, char *redir, t_data *data)
{
	char	**cmd1;
	char	**absolut_path;
	char	**total_env;
